...
4. If Row Level Security is what protects the anon key — what happens if you forget to enable RLS on a table?

> **Answers:** (1) Publishable key (`sb_publishable_...`) is browser-safe — Row Level Security limits what it can access. Secret key (`sb_secret_...`) bypasses all security — server only, never in frontend code. (2) Sessions work differently in browser vs server. (3) Advantage: flexible, no schema changes. Disadvantage: can't query nested fields as easily. (4) Any user with the Publishable key can read ALL rows — a serious security breach.
...
> *"Set up Supabase authentication middleware for my Next.js App Router project. Create: a `src/middleware.ts` file that refreshes the user's session on every request, a `src/app/auth/login/page.tsx` with email/password login form, a `src/app/auth/signup/page.tsx` with email/password signup form, and a redirect so that visiting `/dashboard` while not logged in sends users to `/auth/login`. Use Supabase SSR. Keep the UI minimal for now — we'll style it in Hour 7."*

Let it run completely. This is a substantial amount of code — a middleware file, two page files, possibly a server action file. Let Claude Code finish before asking questions.

### Task 7 — Understand What Was Just Built

Ask Claude Code:

> *"Walk me through what `src/middleware.ts` does — explain each part like I'm a designer who understands flow but not server internals. Specifically: what is middleware, when does it run, and what does 'refreshing a session' mean in practice?"*

Then read the signup and login pages. Identify these patterns in the code:

**In the signup page:**
- Where is the Supabase call? (look for `supabase.auth.signUp`)
- Where does the form handle errors? (look for a `try/catch` or an `error` state)
- What happens after a successful signup? (look for a `redirect` call)

**In the login page:**
- Where is `supabase.auth.signInWithPassword`?
- What is the difference between this call and `signUp`?

If you can't find these patterns, ask Claude Code to point them out — don't move on until you can locate them.
...
```bash
git add .
git commit -m "feat: add Supabase auth — signup, login, signout, middleware session handling"
git push
```
...
2. What does the `middleware.ts` file do, and why does it need to run on every request?
...
5. What would happen to the dashboard if you removed the redirect middleware?
...
Right now, your `agents` table has RLS disabled by default. That means any user with your anon key can query ALL rows in the table — they'd see every agent from every user. This is a critical security flaw.
...
| Insert a row with the wrong user_id in SQL editor | RLS blocks it (permission denied error) | |
...
**Reports:** Usage metrics — requests per day, database size, authentication events. In Week 9 you'll pair this with Vercel Analytics.

Ask Claude Code: *"Give me a one-paragraph overview of what each Supabase product section does — Storage, Edge Functions, Realtime, Logs — and when in my 12-week plan I'd use each one."*
...
LIMIT 10;
```
Understand: what does `JOIN` do? What does `ON agents.user_id = auth.users.id` mean? This is how you'd build an admin view showing which user created which agent.
...
**The anon key is safe only because of RLS.** If RLS is off on any table, your anon key effectively gives read access to your entire database to anyone who can find it. This is the single most common security mistake in Supabase apps. You've now tested this — don't forget what you saw.
...
**Middleware runs before the page.** This is why the auth redirect works — `middleware.ts` intercepts every request before Next.js renders anything. The user never even sees the dashboard HTML if they're not authenticated. This matters for performance and security equally.
...
- [ ] Open `src/middleware.ts` and explain what it does at a high level
